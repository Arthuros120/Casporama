...
create procedure getUserInfoById(iduser integer) as
    Begin
        select * from information where id = iduser;
    END;

create procedure getUserLocationById(iduser int) as
    Begin
...
create procedure addLocalisation( newidadresse int,  newid int,  newname varchar(255), newadresse varchar(255),  newcode int,  newville varchar(255),  newdep varchar(255),  newpays varchar(255)) as
    BEGIN
...
create procedure addProduct( newid int,  newtype varchar(15),  newnusport int,  newmarque varchar(255),  newnom varchar(255), newgenre varchar(5),  newprix float,  newdesc varchar(255),  newimage varchar(255)) as
    BEGIN
...
create procedure addCatalogue( newid int, newreference int , newproduit int,  newcouleur varchar(20),  newtaille varchar(3),  newquantite int) as
    BEGIN
...
create procedure updateEtat( nuorder int, newstate varchar(15)) as
    BEGIN
        update `order` set state=newstate where idorder = nuorder;
    end;

create procedure updateAdresseCommande( nuorder int, newlocation varchar(15)) as
    BEGIN
        update `order` set idlocation=newlocation where idorder = nuorder;
    end;

create procedure updateCoordonnees( iduser int,  newfirstname varchar(255),  newname varchar(255),  newmail varchar(255),  newmobile int,  newfix int) as
    BEGIN
        update information set firstname=newfirstname, name=newname, mail=newmail, mobile=newmobile, fix=newfix where id=iduser;
    end;

create procedure updateLocalisation( newidlocation int, iduser int, newname varchar(255)  ,newlocation varchar(255),  newcode int,  newcity varchar(255),  newdep varchar(255),  newcountry varchar(255)) as
    BEGIN
...
create procedure updatePrice( nuproduct int,  newprice int) as
    BEGIN
        update product set price=newprice where idproduct=nuproduct;
    end;

create procedure updateDescription( nuproduct int,  newdesc varchar(255)) as
    BEGIN
        update product set description=newdesc where idproduct=nuproduct;
    end;

create procedure updateImage( nuproduct int,  newimage varchar(255)) as
    BEGIN
...
create procedure updateStatus( iduser int,  newstate varchar(20)) as
    BEGIN
